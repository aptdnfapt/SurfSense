...
| ENV VARIABLE                    | DESCRIPTION                                                |
| ------------------------------- | ---------------------------------------------------------- |
| NEXT_PUBLIC_FASTAPI_BACKEND_URL | Leave as `/api/proxy` to route browser requests through the frontend proxy |
| BACKEND_BASE_URL                | Actual backend URL used by the proxy (default `http://backend:8000`) |
